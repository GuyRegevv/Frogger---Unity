...
[System.Serializable]
    ...
    private void AssignRandomSprite(GameObject vehicle)
    {
        // Ensure the car has a SpriteRenderer component
        SpriteRenderer spriteRenderer = vehicle.GetComponent<SpriteRenderer>();

        if (spriteRenderer != null && vehicleSprites.Count > 0)
        {
            // Choose a random sprite from the list
            int randomIndex = Random.Range(0, vehicleSprites.Count);
